...
public class ContainerPatternEncoder extends ContainerMachine {
	...
	private InventoryCrafting craftMatrix = new InventoryCrafting(this, TileEntityPatternEncoder.CRAFTING_MATRIX_SIZE,
	...
	protected void addSlots(InventoryPlayer inv, AbstractTileEntityMachine te) {
		// Add crafting result.
		this.addSlotToContainer(new SlotCrafting(inv.player, this.craftMatrix, this.craftResult, 0, 125, 24));
		...
		for (int y = 0; y < 3; y++) {
			for (int x = 0; x < 3; x++) {
				...
				// 67 + y * 18, 6 + x * 18));
				this.addSlotToContainer(new Slot(this.craftMatrix, x + y * 3, 31 + x * 18, 6 + y * 18));
