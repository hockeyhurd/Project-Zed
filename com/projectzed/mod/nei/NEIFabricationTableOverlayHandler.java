 ...
 * @version Feb 5, 2015
 */
public class NEIFabricationTableOverlayHandler implements IOverlayHandler {
	...
	private List<IngredientDistribution> assignIngredients(List<PositionedStack> ingredients, List<DistributedIngred> ingredStacks) {
		...
		for (PositionedStack posstack : ingredients)// assign what we need and
													// have
		{
			...
			for (ItemStack pstack : posstack.items) {
				for (int j = 0; j < ingredStacks.size(); j++) {
					...
					if (!canStack(pstack, istack.stack) || istack.invAmount - istack.distributed < pstack.stackSize) continue;
	...
	private Slot[][] assignIngredSlots(GuiContainer gui, List<PositionedStack> ingredients, List<IngredientDistribution> assignedIngredients) {
		...
		Slot[][] recipeSlots = new Slot[mapIngredSlots(gui, ingredients).length][];

		for (int y = 0; y < mapIngredSlots(gui, ingredients).length; y++) {
			if (mapIngredSlots(gui, ingredients)[y] != null && mapIngredSlots(gui, ingredients)[y].length > 0) {
				for (int x = 0; x < mapIngredSlots(gui, ingredients)[y].length; x++) {
					recipeSlots[y][x] = mapIngredSlots(gui, ingredients)[y][x];
				}
			}
		}
		...
		for (Slot[] recipeSlot : recipeSlots) {
			if (recipeSlot != null && recipeSlot.length > 0) {
	...
	private void findInventoryQuantities(GuiContainer gui, List<DistributedIngred> ingredStacks) {
		...
		for (Slot slot : (List<Slot>) gui.inventorySlots.inventorySlots) {
			if (slot.getHasStack() && (slot.inventory instanceof TileEntityFabricationTable || slot.inventory instanceof InventoryPlayer)) {
	...
	public DistributedIngred findIngred(List<DistributedIngred> ingredStacks, ItemStack pstack) {
		...
		for (DistributedIngred istack : ingredStacks)
			if (canStack(pstack, istack.stack)) return istack;
		return null;
	}
	...
	public Slot[][] mapIngredSlots(GuiContainer gui, List<PositionedStack> ingredients) {
		...
		// TODO: Error in logic somewhere in here!
		...
		for (int i = 0; i < ingredients.size(); i++) {
			...
			for (Slot slot : (List<Slot>) gui.inventorySlots.inventorySlots) {
				...
				if (slot.getHasStack() && slot.getStack().getItem() == pstack.item.getItem()) {
					System.out.println("Adding Slot:\t" + slot.getStack().getDisplayName());
	...
	public static boolean canStack(ItemStack stack1, ItemStack stack2) {
		return stack1 == null
				|| stack2 == null
				|| (stack1.getItem() == stack2.getItem() && (!stack2.getHasSubtypes() || stack2.getItemDamage() == stack1.getItemDamage()) && ItemStack
						.areItemStackTagsEqual(stack2, stack1)) && stack1.isStackable();
	}
