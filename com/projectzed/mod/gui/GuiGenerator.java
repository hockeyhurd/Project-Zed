...
public class GuiGenerator extends GuiContainer implements IInfoContainer {
	...
	public void drawGuiContainerForegroundLayer(int x, int y) {
		...
		this.fontRendererObj.drawString(I18n.format(genSize, new Object[0]), this.xSize / 2 - this.fontRendererObj.getStringWidth(genSize) / 2, this.ySize - 116,
				4210752);
	...
	public void updateScreen() {
		...
		if (this.te != null) {
			this.stored = this.te.getEnergyStored();
			this.maxStorage = this.te.getMaxStorage();
			this.genRate = this.te.getSource().getEffectiveSize() > 0 && this.te.canProducePower() && this.stored + this.te.getSource().getEffectiveSize() <= this.maxStorage ? this.te.getSource().getEffectiveSize() : 0; 
			
			if (getComponents() != null && getComponents().size() > 0) {
				getComponents().get(0).update(new Vector4Helper<Integer>(this.mouseX, this.mouseY, 0), this.stored, this.maxStorage);
			}
		}
	}
	...
	public void initGui() {
		...
		this.labelList.add(new PowerLabel<Integer>(new Vector4Helper<Integer>(guiLeft + 7, guiTop + 61, 0), new Vector4Helper<Integer>(162, 17, 0), this.stored, this.maxStorage));
	}
